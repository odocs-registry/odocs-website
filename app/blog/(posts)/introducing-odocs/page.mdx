...
We've also created a [test repository](https://github.com/odocs-registry/odocs/tree/main/__test__) with sample code that you can run to see the prototype in action. This simple demonstration shows how an AI assistant with access to version-specific documentation generates more accurate, compatible code compared to an assistant without this context.
...
*Special thanks to Docker for inspiring our registry model approach, and to the Hono team for their support and LLM-optimized documentation that made our first prototype possible.*
